...
<c:set var="searchVal" value="${valByName}"/>
...
<c:if test="${empty listByName}">
 ...
 <form action="${root}/member/memberAllList?command=search&searchKey=name&searchVal=${searchVal}&pageNo=1" name="frmAdmin" id="frmAdmin">
...
<c:forEach var="member" items="${listByName}" varStatus="status">
    	<tr> 
    		<td>${member.id}</td>
    		<td>${member.name}</td>
    		<td>${member.age}</td>
    ...
    <tr align="center">
    		...
    		<c:if test="${(startPage-PAGESIZE) gt 0}">
    		 <a href="${root}/member/memberAllList?command=search&searchKey=name&searchVal=${searchVal}&pageNo=${startPage-PAGESIZE}">
    		     ◀ 이전
    		 </a>
    		</c:if>
     ...
     <c:forEach begin="${startPage}" end="${lastPage}" step="1" varStatus="i">
    		 <c:choose>
    		  <c:when test="${i.index == pageNo}">
    		   ...
    		   <c:otherwise>
    		   <a href="${root}/member/memberAllList?command=search&searchKey=name&searchVal=${searchVal}&pageNo=${i.index}">
