...
<c:set var="searchVal" value="${valById}"/>
넘오온 서치 밸류의 값  :  ${searchVal}

<c:if test="${empty listById}">
			<tr>
				<td width="16" height="16" align="right"></td>
				<td colspan="4">등록된 인원이 없습니다.</td>
			</tr>
</c:if>

 <c:if test="${! empty listById}">
 
 <form action="${root}/member/memberAllList?command=search&searchKey=id&searchVal=${valById}&pageNo=1" name="frmAdmin" id="frmAdmin">
...
<c:forEach var="member" items="${listById}" varStatus="status">
    	<tr> 
    		<td>${member.id}</td>
    		<td>${member.name}</td>
    		<td>${member.age}</td>
    ...
    <tr align="center">
      <td colspan="10" style="width:280px;" >	
    		<c:if test="${(startPage-PAGESIZE) gt 0}">
    		 <a href="${root}/member/memberAllList?command=search&searchKey=id&searchVal=${valById}&pageNo=${startPage-PAGESIZE}">
    		     ◀ 이전
    		 </a>
    		</c:if>
    	 ...
    	 <c:forEach begin="${startPage}" end="${lastPage}" step="1" varStatus="i">
    		 <c:choose>
    		  <c:when test="${i.index == pageNo}">
    		   ...
    		   <c:otherwise>
    		   <a href="${root}/member/memberAllList?command=search&searchKey=id&searchVal=${valById}&pageNo=${i.index}">
    		     <font size="4" color="BLUE">${i.index}</font>
    		   </a>
    		   </c:otherwise>
    		 </c:choose>
    		</c:forEach>
